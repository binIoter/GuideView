...
public class Guide implements View.OnKeyListener, View.OnClickListener {
	...
	private MaskView onCreateView(Activity activity) {
		...
		if (mConfiguration.mTargetView != null) {
			maskView.setTargetRect(Common.getViewAbsRect(
			...
			if (target != null) {
				maskView.setTargetRect(Common.getViewAbsRect(target, parentX,
						parentY));
			}
		}
		...
		if (fulling != null) {
			maskView.setFullingRect(Common.getViewAbsRect(fulling, parentX,
					parentY));
		}
		...
		for (Component c : mComponents) {
			maskView.addView(Common.componentToView(
